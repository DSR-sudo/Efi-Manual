    ...
    )
{
    ...
    if (KernelBase == NULL)
    {
        PRINT_KERNEL_PATCH_MSG(L"[!] SetupKiSystemStartupHook: Failed to find ntoskrnl base\r\n");
        return;
    }
    ...
    if (NtHeaders == NULL)
    {
        PRINT_KERNEL_PATCH_MSG(L"[!] SetupKiSystemStartupHook: Invalid NT headers\r\n");
        return;
    }
    ...
    if (KiSystemStartup == NULL)
    {
        PRINT_KERNEL_PATCH_MSG(L"[!] SetupKiSystemStartupHook: KiSystemStartup not found\r\n");
        return;
    }
    
    PRINT_KERNEL_PATCH_MSG(L"[+] Found KiSystemStartup at 0x%p\r\n", KiSystemStartup);
    ...
    if (EFI_ERROR(Status))
    {
        PRINT_KERNEL_PATCH_MSG(L"[!] Failed to allocate trampoline: %r\r\n", Status);
        return;
    }
    ...
    CopyMem(Trampoline, KiSystemStartup, sizeof(gHookTemplate));
    
    // Create JMP back
    UINT8* JmpBackLocation = Trampoline + sizeof(gHookTemplate);
    JmpBackLocation[0] = 0xE9;
    
    UINT8* HookTarget = (UINT8*)KiSystemStartup;
    UINT8* JmpBackDest = HookTarget + sizeof(gHookTemplate);
    INT32 RelOffset = (INT32)((INT64)JmpBackDest - (INT64)JmpBackLocation - 5);
    *(INT32*)(JmpBackLocation + 1) = RelOffset;
    
    gOriginalKiSystemStartup = Trampoline;
    
    // 5. Install Hook pointing to KiSystemStartupStub
    ...
    *(VOID**)(&HookBytes[gHookTemplateAddressOffset]) = (VOID*)KiSystemStartupStub;
    ...
    CopyWpMem(HookTarget, HookBytes, sizeof(HookBytes));
    
    PRINT_KERNEL_PATCH_MSG(L"[+] KiSystemStartup hooked -> 0x%p\r\n", KiSystemStartupStub);
}
